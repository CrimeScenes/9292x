...
local holdingMouseButton = false  -- Variable to track if the mouse button is being held down
    ...
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if shared.Forbidden.Silent.Enabled and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                ...
                -- Silent Aim: Fire RemoteEvent for Silent Aim
                -- Assuming 'RemoteEvent' is the correct RemoteEvent for Silent Aim
                Replicated_Storage.RemoteEvent:FireServer(closestPoint + velocity * Vector3.new(
                    shared.Forbidden.Silent.TimeDistortion.X, 
                    shared.Forbidden.Silent.TimeDistortion.Y, 
                    shared.Forbidden.Silent.TimeDistortion.Z
                ))

                -- TriggerBot Integration
                if holdingMouseButton then
                    TriggerBotAction()  -- Trigger the shot based on silent aim position
                end
            end
        end
    end
...
local function calculateFov(X, Y, Z)
    local baseSize = 3.5  -- The size value for X, Y, Z in the base case
    local baseFov = 12    -- The FOV when X, Y, Z are all 3.5

    -- Calculate the product of the current bounding sizes
    local sizeProduct = X * Y * Z

    -- Calculate the new FOV based on the ratio of the product to the base case
    local calculatedFov = baseFov * (sizeProduct / (baseSize * baseSize * baseSize))

    return calculatedFov
end

-- Modify the updateFov function
local function updateFov()
    local settings = shared.Forbidden.Silent.VoidLimits
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    ...
    -- Dynamically calculate the FOV based on X, Y, Z values
    local dynamicFovSize = calculateFov(shared.Forbidden.Silent.VoidLimits.X, shared.Forbidden.Silent.VoidLimits.Y, shared.Forbidden.Silent.VoidLimits.Z)
...
local OldTimeDistortion = shared.Forbidden.Silent.TimeDistortion  
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        local distortionX = shared.Forbidden.Silent.TimeDistortion.X
        local distortionY = shared.Forbidden.Silent.TimeDistortion.Y
        local distortionZ = shared.Forbidden.Silent.TimeDistortion.Z
        ...
        if adjustedVelocity.Y < -30 and shared.Forbidden.Silent.Resolver then
            shared.Forbidden.Silent.TimeDistortion = { X = 0, Y = 0, Z = 0 } 
            return adjustedVelocity
        elseif adjustedVelocity.Magnitude > 50 and shared.Forbidden.Silent.Resolver then
      
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16 * distortionX  
        else
            shared.Forbidden.Silent.TimeDistortion = OldTimeDistortion 
            return adjustedVelocity
        end
    end
...
UserInputService.InputEnded:Connect(function(input, isProcessed)
    if input.KeyCode == Enum.KeyCode[shared.Forbidden.Target.Keybind:upper()] and shared.Forbidden.pointassist.Method == "hold" then
        holdingMouseButton = false
    end
...
RunService.RenderStepped:Connect(function()
    ...
    if shared.Forbidden.Silent.Enabled and IsTargeting then  
        ...
        -- Force the use of "target" mode regardless of what's in the shared.Forbidden.Silent.mode
        if TargetPlayer then
            if TargetPlayer.Character then
                ...
                if closestPart and closestPoint then
                    ...
                    local timeDistortionX = shared.Forbidden.Silent.TimeDistortion.X
                    local timeDistortionY = shared.Forbidden.Silent.TimeDistortion.Y
                    local timeDistortionZ = shared.Forbidden.Silent.TimeDistortion.Z
...
task.spawn(function()
    while task.wait(0.1) do
        if shared.Forbidden.Silent.Enabled then
            ...
            Fov.Visible = IsTargeting and shared.Forbidden.Silent.VoidLimits.FovVisible 
        end
    end
end)
    ...
    if tool:IsA("Tool") then
        tool.Activated:Connect(function()
            if tick() - lastToolUse > 0.1 then  
                ...
                -- Force "target" mode always
                local target = TargetPlayer  -- Always use the TargetPlayer, regardless of the mode
                ...
                if target and target.Character then
                    ...
                    if closestPart and closestPoint then
                        ...
                        -- Fire the event with the adjusted velocity
                        Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + adjustedVelocity)
                    end
                end
            end
